...
설계: `2026-07-20-refbatch-compile-measurement-design.md`. 하네스: `src/test/java/org/javacs/RefBatchCompileBenchmark.java`. 대상: hub-api(Gradle 멀티모듈, Lombok 사용률 높음). K=1, JDK21, classpath 원본 1회 주입.
...
- **이건 성능 이슈일 뿐 아니라 프로덕션 정확성 버그다.** G(원본 직접, 스테이징 무관)도 9690 errors·9012 unresolved → **커밋된 build/generated·bin 을 가진 레포**에서 sari L5 java 참조가 실제로 ~45% 누락 + 에러 폭증 상태였다.
